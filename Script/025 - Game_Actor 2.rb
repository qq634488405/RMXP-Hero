...
class Game_Actor < Game_Battler
  ...
  #--------------------------------------------------------------------------
  def clear_sword
    @sword_font,@sword_name,@sword_exp,@sword_gold = "","",0,0
  ...
  # ● 生成联机对战数据
  #--------------------------------------------------------------------------
  def make_net_data(type)
    # 卸下武器的属性作为基础属性
    now_weapon = @weapon_id
    if @weapon_id > 0
    ...
    n = type == 0 ? "server_data|" : "client_data|"
    # 各项属性
    n += @name+"|"+@gender.to_s+"|"+@age.to_s+"|"+hit.to_s+"|"+eva.to_s+"|"+
         atk.to_s+"|"+pdef.to_s+"|"+@exp.to_s+"|"+@fp_plus.to_s+@mp_plus.to_s+
         "|"+@base_str.to_s+"|"+@base_agi.to_s+"|"+@base_int.to_s+"|"+
         @base_bon.to_s+"|"+@base_fac.to_s+"|"+luck.to_s+"|"+@hp.to_s+"|"+
         @maxhp.to_s+"|"+@fp.to_s+"|"+@maxfp.to_s+"|"+@mp.to_s+"|"+
         @maxmp.to_s+"|"+full_hp.to_s+"|"+now_weapon.to_s+"|"
    # 装备的功夫
    for i in 0..5
      n += @skill_use[i] + "|"
    end
    # 功夫列表
    n += @skill_list.size.to_s
    ...
    n += "|"+@sword_name+"|"+@sword_type.to_s+"|"+@sword1.to_s+"|"+@sword2.to_s+
    "|"+@sword3.to_s
    equip(1,bag_id) if need_equip
    return n
  end
end