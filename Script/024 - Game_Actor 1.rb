...
class Game_Actor < Game_Battler
  ...
  attr_accessor :marry_crc                # 对象加密信息(确认是否与对象联机)
  attr_accessor :couple_kf_list           # 对象技能列表
  ...
  attr_accessor :room_level               # 桃花源房屋等级
  attr_accessor :jiaju_list               # 桃花源家具列表
  #--------------------------------------------------------------------------
  # ● 初始化对像
  #--------------------------------------------------------------------------
  def initialize
    ...
    @sword_exp,@sword_gold,@kill_num,@jiaju_list = 0,0,0,[0,0,0,0,0]
    @donate_times,@have_new_home,@room_level,@marry_crc = 0,false,0,nil
    @couple_kf_list = []
  end
  #--------------------------------------------------------------------------
  # ● 获取角色 ID 
  ...
  def gain_item(type,id,number=1)
    return unless can_get_item?(type,id,number)
    # 类型不是物品的情况
    if type != 1
