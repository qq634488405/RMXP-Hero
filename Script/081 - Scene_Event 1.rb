...
class Scene_Event
  ...
  #           11--喝酒
  #           12--联机对战
  #           13--跳转XX坛地图，id--地图ID，number--入口ID
  #           14--铸剑
  #           15--桃花源
  #           16--进入桃花源小屋
  #--------------------------------------------------------------------------
  ...
  def npc_event
    ...
    if @id == 172 # 老管家
      @phase = 11
      @master_step = 1
      @npc_menu=Window_Command.new(104,$data_system.master_menu,1,2,2)
      h = $data_system.master_menu.size * 32 + 32
    elsif @id == 100000 # 夫妻
      $data_enemies[200] = RPG::Enemy.new
      $data_enemies[200].name = @actor.marry_name
      @phase = 13
      @connected = false
      @marry_step = 1
      @npc = Game_Enemy.new(200)
      @npc_menu=Window_Command.new(104,$data_system.couple_menu,1,2,2)
      h = $data_system.couple_menu.size * 32 + 32
    else
      @npc_menu=Window_Command.new(104,$data_system.menu_type[@menu],1,2,2)
      h = $data_system.menu_type[@menu].size * 32 + 32
      @npc_step=0
    end
  ...
  def set_npc_menu(h)
    ...
    x=[[$game_player.screen_x+x_offset,536].min,0].max
    y=[[$game_player.screen_y+y_offset,480-h].min,24].max
  ...
  def drink_wine
    ...
    # 闪烁
    color = $color_mode == 0 ? Color.new(144,176,87,255) : Color.new(204,204,204,255)
    $game_screen.start_flash(color,50)
    # 任务时间增加
  ...
  def go_to_home
    # 获取坐标
    id,x,y = $data_system.home_position[@actor.room_level - 1]
    move_to_map(id,x,y)
  end
  #--------------------------------------------------------------------------
  # ● 移动至指定地图位置
